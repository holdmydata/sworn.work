...
export const user_stats = pgTable('user_stats', {
	...
	quests_completed: integer('quests_completed').notNull().default(0),
	reviews_received: integer('reviews_received').notNull().default(0),
...
export const badges = pgTable('badges', {
	...
	badge_type: varchar('badge_type', { length: 40 }).notNull(),
	category_slug: varchar('category_slug', { length: 100 }),
...
export const messages = pgTable('messages', {
	...
	sender_id: serial('sender_id')
		.notNull()
		.references(() => users.id),
	recipient_id: serial('recipient_id')
		.notNull()
		.references(() => users.id),
...
export const reviews = pgTable('reviews', {
	...
	reviewer_id: serial('reviewer_id')
		.notNull()
		.references(() => users.id),
	reviewee_id: serial('reviewee_id')
		.notNull()
		.references(() => users.id),
	task_id: serial('task_id')
		.notNull()
...
export const payments = pgTable('payments', {
	...
	task_id: serial('task_id')
		.notNull()
		.references(() => tasks.id),
	payer_id: serial('payer_id')
		.notNull()
		.references(() => users.id),
	payee_id: serial('payee_id')
		.notNull()
		.references(() => users.id),
...
export const task_proofs = pgTable('task_proofs', {
	...
	task_id: serial('task_id')
		.notNull()
		.references(() => tasks.id, { onDelete: 'cascade' }),
	submitted_by_user_id: serial('submitted_by_user_id')
		.notNull()
		.references(() => users.id, { onDelete: 'cascade' }),
...
export const verification_decisions = pgTable('verification_decisions', {
	...
	decided_by_user_id: serial('decided_by_user_id')
		.notNull()
		.references(() => users.id, { onDelete: 'cascade' }),
...
export const user_badges = pgTable(
	'user_badges',
	{
		...
		user_id: integer('user_id')
			.notNull()
			.references(() => users.id, { onDelete: 'cascade' }),
		badge_key: varchar('badge_key', { length: 100 })
			.notNull()
			.references(() => badges.key, { onDelete: 'cascade' }),
		earned_at: timestamp('earned_at').notNull().defaultNow(),
		source_task_id: integer('source_task_id').references(() => tasks.id, { onDelete: 'set null' })
	},
	(table) => ({
		userBadgeUnique: uniqueIndex('user_badges_user_id_badge_key_unique').on(
			table.user_id,
			table.badge_key
		)
	})
...
export const usersRelations = relations(users, ({ one, many }) => ({
	...
	earned_badges: many(user_badges),
	tasks: many(tasks),
	bids: many(bids),
...
export const userStatsRelations = relations(user_stats, ({ one }) => ({
	user: one(users, { fields: [user_stats.user_id], references: [users.id] })
}));

export const badgesRelations = relations(badges, ({ many }) => ({
	users: many(user_badges)
}));

export const userBadgesRelations = relations(user_badges, ({ one }) => ({
	user: one(users, { fields: [user_badges.user_id], references: [users.id] }),
	badge: one(badges, { fields: [user_badges.badge_key], references: [badges.key] }),
	source_task: one(tasks, { fields: [user_badges.source_task_id], references: [tasks.id] })
}));
...
export const verificationDecisionsRelations = relations(verification_decisions, ({ one }) => ({
	...
	proof: one(task_proofs, {
		fields: [verification_decisions.proof_id],
		references: [task_proofs.id]
	}),
	decided_by: one(users, {
		fields: [verification_decisions.decided_by_user_id],
		references: [users.id]
	})
}));
